...
struct IndexFilteredHNSW;

struct ReconstructFromNeighbors2 {
    typedef FilteredHNSW::storage_idx_t storage_idx_t;
    ...
    /// codes must be added in the correct order and the IndexFilteredHNSW
...
struct IndexFilteredHNSW : Index {
    typedef FilteredHNSW::storage_idx_t storage_idx_t;
    ...
    explicit IndexFilteredHNSW(int d = 0, int M = 32, int gamma = 1, MetricType metric = METRIC_L2);
    explicit IndexFilteredHNSW(Index* storage, int M = 32, int gamma = 1);

    ~IndexFilteredHNSW() override;
...
struct IndexFilteredHNSWFlat : IndexFilteredHNSW {
    IndexFilteredHNSWFlat();
    IndexFilteredHNSWFlat(int d, int M, int gamma = 1, MetricType metric = METRIC_L2);
};

/** PQ index topped with with a HNSW structure to access elements
 *  more efficiently.
 */
struct IndexFilteredHNSWPQ : IndexFilteredHNSW {
    IndexFilteredHNSWPQ();
    IndexFilteredHNSWPQ(int d, int pq_m, int M);
    void train(idx_t n, const float* x) override;
};
 ...
 *  more efficiently.
 */
struct IndexFilteredHNSWSQ : IndexFilteredHNSW {
    IndexFilteredHNSWSQ();
    IndexFilteredHNSWSQ(
            int d,
 ...
 */
struct IndexFilteredHNSW2Level : IndexFilteredHNSW {
    IndexFilteredHNSW2Level();
    IndexFilteredHNSW2Level(Index* quantizer, size_t nlist, int m_pq, int M);
 ...
 * HYBRID INDEX
 **************************************************************/
/** Inherits from IndexFilteredHNSWFlat
 */
// TODO - change metadata from int* to generic type
struct IndexFilteredHNSWHybridOld : IndexFilteredHNSWFlat {
    IndexFilteredHNSWHybridOld();
    IndexFilteredHNSWHybridOld(int d, int M, int gamma, std::vector<int>& metadata, MetricType metric = METRIC_L2);
