...
static void write_FilteredHNSW(const FilteredHNSW* hnsw, IOWriter* f) {
    WRITEVECTOR(hnsw->assign_probas);
    WRITEVECTOR(hnsw->cum_nneighbor_per_level);
    WRITEVECTOR(hnsw->levels);
    WRITEVECTOR(hnsw->offsets);
...
static void write_NaviX(const NaviX* hnsw, IOWriter* f) {
    WRITEVECTOR(hnsw->assign_probas);
    WRITEVECTOR(hnsw->cum_nneighbor_per_level);
    WRITEVECTOR(hnsw->levels);
    WRITEVECTOR(hnsw->offsets);
...
static void write_PathSeer(const PathSeer* pathseer, IOWriter* f) {
    WRITEVECTOR(pathseer->assign_probas);
    WRITEVECTOR(pathseer->cum_nneighbor_per_level);
    WRITEVECTOR(pathseer->levels);
    WRITEVECTOR(pathseer->offsets);
    WRITEVECTOR(pathseer->neighbors);
    WRITEVECTOR(pathseer->neighbor_boundaries);
    WRITEVECTOR(pathseer->offset_neighbor);

    WRITE1(pathseer->entry_point);
    WRITE1(pathseer->max_level);
    WRITE1(pathseer->efConstruction);
    WRITE1(pathseer->efSearch);
    WRITE1(pathseer->upper_beam);

    // added for hybrid version
    WRITE1(pathseer->M);
    WRITE1(pathseer->M_exp);
    WRITE1(pathseer->use_only_expanding_idx);
...
void write_index(const Index* idx, IOWriter* f) {
    ...
    else if (const IndexFilteredHNSW* idxfilteredhnsw = dynamic_cast<const IndexFilteredHNSW*>(idx)) {
        uint32_t h = dynamic_cast<const IndexFilteredHNSWFlat*>(idx) ? fourcc("IFHN") : 0;
        FAISS_THROW_IF_NOT(h != 0);
        WRITE1(h);
        write_index_header(idxfilteredhnsw, f);
        write_FilteredHNSW(&idxfilteredhnsw->hnsw, f);
        write_index(idxfilteredhnsw->storage, f);
    }
    else if (const IndexNaviXFlat* idxnavix = dynamic_cast<const IndexNaviXFlat*>(idx)) {
        uint32_t h = dynamic_cast<const IndexNaviXFlat*>(idx) ? fourcc("IFNX") : 0;
        FAISS_THROW_IF_NOT(h != 0);
        WRITE1(h);
        write_index_header(idxnavix, f);
        write_NaviX(&idxnavix->navix, f);
        write_index(idxnavix->storage, f);
    }
    ...
    else if (const IndexPathSeer* indxpathseer = dynamic_cast<const IndexPathSeer*>(idx)) {
        uint32_t h = fourcc("IHMA"); // this needs to be a 4 letter header
        FAISS_THROW_IF_NOT(h != 0);
        WRITE1(h);
        write_index_header(indxpathseer, f);
        write_PathSeer(&indxpathseer->pathseer, f);
        write_index(indxpathseer->storage, f);
    }
