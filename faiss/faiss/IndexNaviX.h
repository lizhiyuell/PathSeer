...
struct ReconstructFromNeighborsNaviX {//lbj+rename
        
    typedef NaviX::storage_idx_t storage_idx_t;
    ...
    explicit ReconstructFromNeighborsNaviX(
            const IndexNaviX& index,
            size_t k = 256,
            size_t nsq = 1);

    /// codes must be added in the correct order and the IndexNaviX
...
/** The NaviX index is a normal random-access index with a NaviX
 * link structure built on top */

struct IndexNaviX : Index {
    typedef NaviX::storage_idx_t storage_idx_t;
    ...
    ReconstructFromNeighborsNaviX* reconstruct_from_neighbors;

    explicit IndexNaviX(int d = 0, int M = 32, MetricType metric = METRIC_L2);
    explicit IndexNaviX(Index* storage, int M = 32);
...
/** Flat index topped with with a NaviX structure to access elements
 *  more efficiently.
 */

struct IndexNaviXFlat : IndexNaviX {
    IndexNaviXFlat();
    IndexNaviXFlat(int d, int M, MetricType metric = METRIC_L2);
};

/** PQ index topped with with a NaviX structure to access elements
 *  more efficiently.
 */
struct IndexNaviXPQ : IndexNaviX {
    IndexNaviXPQ();
    IndexNaviXPQ(int d, int pq_m, int M);
    void train(idx_t n, const float* x) override;
};

/** SQ index topped with with a NaviX structure to access elements
 *  more efficiently.
 */
struct IndexNaviXSQ : IndexNaviX {
    IndexNaviXSQ();
    IndexNaviXSQ(
            int d,
 ...
 */
struct IndexNaviX2Level : IndexNaviX {
    IndexNaviX2Level();
    IndexNaviX2Level(Index* quantizer, size_t nlist, int m_pq, int M);
