...
#include <faiss/IndexFilteredHNSW.h>
...
using MinimaxHeap = FilteredHNSW::MinimaxHeap;
using storage_idx_t = FilteredHNSW::storage_idx_t;
using NodeDistFarther = FilteredHNSW::NodeDistFarther;

FilteredHNSWStats filteredhnsw_stats;
...
void hnsw_add_vertices(
    ...
    FilteredHNSW& hnsw = index_hnsw.hnsw;
    size_t ntotal = n0 + n;
...
/**************************************************************
 * IndexFilteredHNSW implementation
 **************************************************************/

IndexFilteredHNSW::IndexFilteredHNSW(int d, int M, int gamma, MetricType metric)
        : Index(d, metric),
          hnsw(M, gamma),
          own_fields(false),
          storage(nullptr),
...
IndexFilteredHNSW::IndexFilteredHNSW(Index* storage, int M, int gamma)
...
IndexFilteredHNSW::~IndexFilteredHNSW() {
    if (own_fields) {
        delete storage;
    }
}

void IndexFilteredHNSW::train(idx_t n, const float* x) {
    FAISS_THROW_IF_NOT_MSG(
            storage,
            "Please use IndexFilteredHNSWFlat (or variants) instead of IndexFilteredHNSW directly");
    // hnsw structure does not require training
    storage->train(n, x);
    is_trained = true;
}

void IndexFilteredHNSW::search(
        idx_t n,
        const float* x,
        idx_t k,
        float* distances,
    ...
    FAISS_THROW_IF_NOT_MSG(
            storage,
            "Please use IndexFilteredHNSWFlat (or variants) instead of IndexFilteredHNSW directly");
    const SearchParametersFilteredHNSW* params = nullptr;

    int efSearch = hnsw.efSearch;
    if (params_in) {
        params = dynamic_cast<const SearchParametersFilteredHNSW*>(params_in);
...
#pragma omp for reduction(+ : n1, n2, n3, ndis, nreorder)
            for (idx_t i = i0; i < i1; i++) {
                ...
                FilteredHNSWStats stats = hnsw.search(*dis, k, idxi, simi, vt, params);
                n1 += stats.n1;
                n2 += stats.n2;
                n3 += stats.n3;
                ndis += stats.ndis;
    ...
    filteredhnsw_stats.combine({n1, n2, n3, ndis, nreorder});
...
void IndexFilteredHNSW::partition_search(
        idx_t n,
        const float* x,
        idx_t k,
        std::vector<faiss::IndexFilteredHNSW*>& partition_indices,
        int num_partitions,
        float* distances,
        idx_t* labels,
        int* filters,
...
#pragma omp for reduction(+ : n1, n2, n3, ndis, nreorder)
            for (idx_t i = i0; i < i1; i++) {
                ...
                FilteredHNSWStats stats = partition_indices[filters[i]]->hnsw.search(*dis_array[filters[i]], k, idxi, simi, vt, params);
                // FilteredHNSWStats stats = hnsw.search(*dis, k, idxi, simi, vt, params);
                n1 += stats.n1;
                n2 += stats.n2;
                n3 += stats.n3;
...
void IndexFilteredHNSW::add(idx_t n, const float* x) {
...
void IndexFilteredHNSW::reset() {
    hnsw.reset();
    storage->reset();
    ntotal = 0;
}

void IndexFilteredHNSW::reconstruct(idx_t key, float* recons) const {
    storage->reconstruct(key, recons);
}

void IndexFilteredHNSW::shrink_level_0_neighbors(int new_size) {
#pragma omp parallel
    {
        DistanceComputer* dis = storage_distance_computer(storage);
        ScopeDeleter1<DistanceComputer> del(dis);
...
#pragma omp for
        for (idx_t i = 0; i < ntotal; i++) {
            ...
            FilteredHNSW::shrink_neighbor_list(
...
void IndexFilteredHNSW::search_level_0(
        idx_t n,
        const float* x,
        idx_t k,
...
#pragma omp parallel
    {
        ...
        FilteredHNSWStats search_stats;
        VisitedTable vt(ntotal);
...
#pragma omp critical
        { filteredhnsw_stats.combine(search_stats); }
    }
}

void IndexFilteredHNSW::init_level_0_from_knngraph(
        int k,
        const float* D,
        const idx_t* I) {
...
#pragma omp parallel for
    for (idx_t i = 0; i < ntotal; i++) {
        ...
        std::vector<NodeDistFarther> shrunk_list;
        FilteredHNSW::shrink_neighbor_list(*qdis, initial_list, shrunk_list, dest_size);
...
void IndexFilteredHNSW::init_level_0_from_entry_points(
        int n,
...
void IndexFilteredHNSW::reorder_links() {
...
void IndexFilteredHNSW::link_singletons() {
...
// added for debugging
void IndexFilteredHNSW::printStats(bool print_edge_list, bool is_hybrid) {
...
/**************************************************************
 * ReconstructFromNeighbors2 implementation
 **************************************************************/

ReconstructFromNeighbors2::ReconstructFromNeighbors2(
        const IndexFilteredHNSW& index,
        size_t k,
        size_t nsq)
...
void ReconstructFromNeighbors2::reconstruct(
        ...
        float* x,
        float* tmp) const {
    const FilteredHNSW& hnsw = index.hnsw;
    size_t begin, end;
    hnsw.neighbor_range(i, 0, &begin, &end);
...
/**************************************************************
 * IndexFilteredHNSWFlat implementation
 **************************************************************/

IndexFilteredHNSWFlat::IndexFilteredHNSWFlat() {
    is_trained = true;
}

IndexFilteredHNSWFlat::IndexFilteredHNSWFlat(int d, int M, int gamma, MetricType metric)
        : IndexFilteredHNSW(new IndexFlat(d, metric), M, gamma) {
    own_fields = true;
    is_trained = true;
}

/**************************************************************
 * IndexFilteredHNSWPQ implementation
 **************************************************************/

IndexFilteredHNSWPQ::IndexFilteredHNSWPQ() {}

IndexFilteredHNSWPQ::IndexFilteredHNSWPQ(int d, int pq_m, int M)
        : IndexFilteredHNSW(new IndexPQ(d, pq_m, 8), M) {
    own_fields = true;
    is_trained = false;
}

void IndexFilteredHNSWPQ::train(idx_t n, const float* x) {
    IndexFilteredHNSW::train(n, x);
    (dynamic_cast<IndexPQ*>(storage))->pq.compute_sdc_table();
}

/**************************************************************
 * IndexFilteredHNSWSQ implementation
 **************************************************************/

IndexFilteredHNSWSQ::IndexFilteredHNSWSQ(
        int d,
        ScalarQuantizer::QuantizerType qtype,
        int M,
        MetricType metric)
        : IndexFilteredHNSW(new IndexScalarQuantizer(d, qtype, metric), M) {
    is_trained = false;
    own_fields = true;
}
...
/**************************************************************
 * IndexFilteredHNSW2Level implementation
 **************************************************************/

IndexFilteredHNSW2Level::IndexFilteredHNSW2Level(
        Index* quantizer,
        size_t nlist,
        int m_pq,
        int M)
        : IndexFilteredHNSW(new Index2Layer(quantizer, nlist, m_pq), M) {
    own_fields = true;
    is_trained = false;
}
...
void IndexFilteredHNSW2Level::search(
        idx_t n,
        const float* x,
        idx_t k,
        float* distances,
    ...
    if (dynamic_cast<const Index2Layer*>(storage)) {
        IndexFilteredHNSW::search(n, x, k, distances, labels);
...
void IndexFilteredHNSW2Level::flip_to_ivf() {
...
/**************************************************************
 * IndexFilteredHNSWHybrid implementation
 **************************************************************/

IndexFilteredHNSWHybridOld::IndexFilteredHNSWHybridOld() {
    is_trained = true;
}

// hybrid HNSW only implemented using Flat storage
IndexFilteredHNSWHybridOld::IndexFilteredHNSWHybridOld(int d, int M, int gamma, std::vector<int>& metadata, MetricType metric)
        : IndexFilteredHNSWFlat(d, M, gamma, metric) { 
    own_fields = true;
    is_trained = true;
...
// add n vectors of dimension d to the index, x is the matrix of vectors
void IndexFilteredHNSWHybridOld::add(idx_t n, const float* x) {
...
// this is the same as search but it calls FilteredHNSW::hybrid_search, instead of FilteredHNSW::search
void IndexFilteredHNSWHybridOld::search(
            idx_t n,
            const float* x,
            idx_t k,
            float* distances,
...
#pragma omp for reduction(+ : n1, n2, n3, ndis, nreorder)
            for (idx_t i = i0; i < i1; i++) {
                ...
                FilteredHNSWStats stats = hnsw.hybrid_search(*dis, k, idxi, simi, vt, filters[i], op, regex, params); //TODO edit to hybrid search
                n1 += stats.n1;
                n2 += stats.n2;
                n3 += stats.n3;
                ndis += stats.ndis;
