...
// three-version-advanced visited table. For each round, one vector has four states, named intact, filtered-fail, filtered-pass
/// set implementation optimized for fast access.
struct VisitedTable {
    std::vector<uint8_t> visited;
    int visno;

    explicit VisitedTable(int size) : visited(size), visno(2) {}

    // functions used for ordinary VT
    void set(int no) {
        visited[no] = visno;
    }
    ...
    // whether the vector is visited
    bool get_either(int no) const {
        return visited[no] >= visno-1;
    }

    /// reset all flags to false
    void advance_two_states() {
        visno+=2;
        if (visno == 250) {
            // 252 rather than 255 because sometimes we use visno and visno+1
            memset(visited.data(), 0, sizeof(visited[0]) * visited.size());
            visno = 2;
        }
    }
