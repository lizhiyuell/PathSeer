...
struct SearchParametersACORN : SearchParameters {
    ...
    // for experiment purpose, limited_mode does not expanding the neighbors
    bool limited_mode = false;
    // whether optimize the number of filters by cutting down repeated filters
    bool filter_opt = true;
    // wehter output the search track
...
struct ACORNStats {
    ...
    ACORNStats(
            ...
            double skips = 0.0,
            double visits = 0.0)
            : n1(n1), n2(n2), n3(n3), ndis(ndis), ndis_first(ndis_first), nfilter(nfilter), nfilter_first(nfilter_first), nhop(nhop), nhop_first(nhop_first), ngood(ngood), ngood_first(ngood_first), nreorder(nreorder), useful_rate(useful_rate), candidates_loop(candidates_loop), neighbors_loop(neighbors_loop), tuple_unwrap(tuple_unwrap), skips(skips), visits(visits) {}
