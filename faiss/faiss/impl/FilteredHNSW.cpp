...
#include <faiss/impl/FilteredHNSW.h>
...
int FilteredHNSW::nb_neighbors(int layer_no) const {
    return cum_nneighbor_per_level[layer_no + 1] -
            cum_nneighbor_per_level[layer_no];
}

void FilteredHNSW::set_nb_neighbors(int level_no, int n) {
...
int FilteredHNSW::cum_nb_neighbors(int layer_no) const {
    return cum_nneighbor_per_level[layer_no];
}

void FilteredHNSW::neighbor_range(idx_t no, int layer_no, size_t* begin, size_t* end)
        const {
    size_t o = offsets[no];
    // debug("offset o: %ld\n", o);
...
// FilteredHNSW::HNSW(int M) : rng(12345) {
...
FilteredHNSW::FilteredHNSW(int M, int gamma) : rng(12345) {
...
void FilteredHNSW::set_default_probas(int M, float levelMult, int gamma) {
    int nn = 0;
...
void FilteredHNSW::clear_neighbor_tables(int level) {
...
void FilteredHNSW::print_neighbor_stats(int level) const {
...
void FilteredHNSW::hybrid_print_neighbor_stats(int level) const {
    FAISS_THROW_IF_NOT(level < cum_nneighbor_per_level.size());
    printf("* stats on level %d, max %d neighbors per vertex:\n",
           level,
           nb_neighbors(level));
...
void FilteredHNSW::print_edges(int level) const {
    FAISS_THROW_IF_NOT(level < cum_nneighbor_per_level.size());
    printf("* stats on level %d, max %d neighbors per vertex:\n",
           level,
           nb_neighbors(level));
...
// same as print_neighbor_stats with additional edge lists printed, no parallelism
void FilteredHNSW::hybrid_print_edges(int level) const {
    FAISS_THROW_IF_NOT(level < cum_nneighbor_per_level.size());
    printf("* stats on level %d, max %d neighbors per vertex:\n",
           level,
           nb_neighbors(level));
...
void FilteredHNSW::print_neighbor_stats(bool edge_list, bool is_hybrid) const {
    printf("========= METADATA =======\n");
...
void FilteredHNSW::fill_with_random_links(size_t n) {
...
int FilteredHNSW::prepare_level_tab(size_t n, bool preset_levels) {
 ...
 */
void FilteredHNSW::shrink_neighbor_list(
...
using NodeDistCloser = FilteredHNSW::NodeDistCloser;
...
void shrink_neighbor_list(
    ...
    FilteredHNSW::shrink_neighbor_list(qdis, resultSet, returnlist, max_size, gamma);
...
void FilteredHNSW::add_links_starting_from(
...
/// Finds neighbors and builds links with them, starting from an entry
/// point. The own neighbor list is assumed to be locked.
void FilteredHNSW::hybrid_add_links_starting_from(
        DistanceComputer& ptdis,
        storage_idx_t pt_id,
        storage_idx_t nearest,
        float d_nearest,
...
void FilteredHNSW::add_with_locks(
        DistanceComputer& ptdis,
        int pt_level,
        int pt_id,
...
void FilteredHNSW::hybrid_add_with_locks(
        DistanceComputer& ptdis,
        int pt_level,
        int pt_id,
        std::vector<omp_lock_t>& locks,
...
std::priority_queue<FilteredHNSW::Node> search_from_candidate_unbounded(
        const FilteredHNSW& hnsw,
        const Node& node,
        DistanceComputer& qdis,
        int ef,
...
FilteredHNSWStats FilteredHNSW::search(
        DistanceComputer& qdis,
        int k,
        idx_t* I,
        float* D,
        VisitedTable& vt,
        const SearchParametersFilteredHNSW* params) const {
    // debug("%s\n", "reached");
    FilteredHNSWStats stats;
    if (entry_point == -1) {
        return stats;
...
// hybrid search TODO
FilteredHNSWStats FilteredHNSW::hybrid_search(
        DistanceComputer& qdis,
        int k,
        idx_t* I,
        float* D,
...
void FilteredHNSW::search_level_0(
        DistanceComputer& qdis,
        int k,
        idx_t* idxi,
        float* simi,
...
void FilteredHNSW::MinimaxHeap::push(storage_idx_t i, float v) {
    if (k == n) {
        if (v >= dis[0])
            return;
...
float FilteredHNSW::MinimaxHeap::max() const {
    return dis[0];
}

int FilteredHNSW::MinimaxHeap::size() const {
    return nvalid;
}

void FilteredHNSW::MinimaxHeap::clear() {
    nvalid = k = 0;
}

int FilteredHNSW::MinimaxHeap::pop_min(float* vmin_out) {
    assert(k > 0);
...
int FilteredHNSW::MinimaxHeap::count_below(float thresh) {
    int n_below = 0;
    for (int i = 0; i < k; i++) {
        if (dis[i] < thresh) {
            n_below++;
