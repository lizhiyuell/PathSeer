...
static void read_FilteredHNSW(FilteredHNSW* hnsw, IOReader* f) {
    READVECTOR(hnsw->assign_probas);
    READVECTOR(hnsw->cum_nneighbor_per_level);
    READVECTOR(hnsw->levels);
    READVECTOR(hnsw->offsets);
...
static void read_NaviX(NaviX* navix, IOReader* f) {
    READVECTOR(navix->assign_probas);
    READVECTOR(navix->cum_nneighbor_per_level);
    READVECTOR(navix->levels);
    READVECTOR(navix->offsets);
    READVECTOR(navix->neighbors);

    READ1(navix->entry_point);
    READ1(navix->max_level);
    READ1(navix->efConstruction);
    READ1(navix->efSearch);
    READ1(navix->upper_beam);
}
...
static void read_PathSeer(PathSeer* pathseer, IOReader* f) {
    READVECTOR(pathseer->assign_probas);
    READVECTOR(pathseer->cum_nneighbor_per_level);
    READVECTOR(pathseer->levels);
    READVECTOR(pathseer->offsets);
    READVECTOR(pathseer->neighbors);
    READVECTOR(pathseer->neighbor_boundaries);
    READVECTOR(pathseer->offset_neighbor);
    
    READ1(pathseer->entry_point);
    READ1(pathseer->max_level);
    READ1(pathseer->efConstruction);
    READ1(pathseer->efSearch);
    READ1(pathseer->upper_beam);
    READ1(pathseer->M);
    READ1(pathseer->M_exp);
    READ1(pathseer->use_only_expanding_idx);
}
...
Index* read_index(IOReader* f, int io_flags) {
    ...
    if (h == fourcc("IxFI") || h == fourcc("IxF2") || h == fourcc("IxFl")) {
        ...
        idx = idxacorn;
    } else if (h == fourcc("IHMA")) {
        // IndexHNSWFlat* idxhnswhybrid = new IndexHNSWFlat();
        IndexPathSeer* idxpathseer= nullptr;
        idxpathseer = new IndexPathSeerFlat(0, 0, 0);
        // IndexACORNFlat* idxhnsw = new IndexACORNFlat();
        // IndexACORN* idxhnsw = new IndexACORNFlat();
        read_index_header(idxpathseer, f);
        read_PathSeer(&idxpathseer->pathseer, f);
        idxpathseer->storage = read_index(f, io_flags);
        idxpathseer->own_fields = true;
        idx = idxpathseer;
    } else if (h == fourcc("IFHN")) {
        IndexFilteredHNSWFlat* idxfilteredhnsw = new IndexFilteredHNSWFlat();
        read_index_header(idxfilteredhnsw, f);
        read_FilteredHNSW(&idxfilteredhnsw->hnsw, f);
        idxfilteredhnsw->storage = read_index(f, io_flags);
        idxfilteredhnsw->own_fields = true;
        idx = idxfilteredhnsw;
    } else if (h == fourcc("IFNX")) {
        IndexNaviXFlat* idxnavix = new IndexNaviXFlat();
        read_index_header(idxnavix, f);
        read_NaviX(&idxnavix->navix, f);
        idxnavix->storage = read_index(f, io_flags);
        idxnavix->own_fields = true;
        idx = idxnavix;
    } else if (
