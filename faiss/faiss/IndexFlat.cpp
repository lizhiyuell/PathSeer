...
using PrefetchFunc = void(*)(const float*, idx_t);

void prefetch_1(const float* base, idx_t i){
    __builtin_prefetch(base, 0, 1);
}

void prefetch_2(const float* base, idx_t i){
    __builtin_prefetch(base, 0, 1);
    __builtin_prefetch(base + 64 / sizeof(float), 0, 1);
}

void prefetch_3(const float* base, idx_t i){
    __builtin_prefetch(base, 0, 1);
    __builtin_prefetch(base + 64 / sizeof(float), 0, 1);
    __builtin_prefetch(base + 128 / sizeof(float), 0, 1);
}

void prefetch_4(const float* base, idx_t i){
    __builtin_prefetch(base, 0, 1);
    __builtin_prefetch(base + 64 / sizeof(float), 0, 1);
    __builtin_prefetch(base + 128 / sizeof(float), 0, 1);
    __builtin_prefetch(base + 192 / sizeof(float), 0, 1);
}
...
struct FlatIPDis : FlatCodesDistanceComputer {
    ...
    explicit FlatIPDis(const IndexFlat& storage, const float* q = nullptr)
            : FlatCodesDistanceComputer(
              ...
              b(storage.get_xb()),
              ndis(0) {
                int prefetch_step = storage.d * sizeof(float) / CACHELINE_SIZE;
                if(prefetch_step<=1)
                    pf = prefetch_1;
                else if(prefetch_step<=2){
                    pf = prefetch_2;
                }
                else if(prefetch_step<=3){
                    pf = prefetch_3;
                }
                else{
                    pf = prefetch_4;
    ...
    void prefetch_vector(idx_t i) {
        const float* base = b + i * d;
        pf(base, i);
    }
};
