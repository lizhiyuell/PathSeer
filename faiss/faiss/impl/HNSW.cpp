...
#pragma omp critical
    ...
    for (; level > pt_level; level--) {
        greedy_update_nearest(*this, ptdis, level, nearest, d_nearest, false);
    }
...
HNSWStats HNSW::search(
    ...
    if (upper_beam == 1) {
        ...
        for (int level = max_level; level >= 1; level--) {
            HNSWStats local_stats = greedy_update_nearest(*this, qdis, level, nearest, d_nearest, params->output_search);
