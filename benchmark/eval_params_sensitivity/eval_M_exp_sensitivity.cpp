...
int main() {
    ...
    // we don't need to test Mexp=128 here as it will be tested in the M_sensitivity test
    std::vector<std::vector<int>> M_exp_construct_params = {
        {64, 96},
        {64, 96},
        {64, 96},
        {64, 96},
    ...
    for (int ds_idx = 0; ds_idx < dataset_array.size(); ds_idx++) {
        ...
        for(int M_exp_construct : M_exp_construct_params[ds_idx]){
