...
static inline bool get_and_set_member(faiss::IDSelector*sel, faiss::VisitedTable& vt, int64_t v, int& nr_filter, int pos){
...
ACORNStats greedy_update_nearest(
        const ACORN& hnsw,
        DistanceComputer& qdis,
        int level,
...
ACORNStats hybrid_greedy_update_nearest(
        const ACORN& hnsw,
        DistanceComputer& qdis,
        IDSelector* sel,
        int level,
    ...
    for (;;) {
        ...
        for (size_t i = begin; i < end; i++) {
            ...
            nfilter++;
            bool pass_filter = sel->is_member(v, pos);
            ...
            if(pass_filter){
                ...
                ndis += 1;
                nfilter++;
                if (dis < d_nearest || !sel->is_member(nearest, pos)) {
                    nearest = v;
                    d_nearest = dis;
                    ngood++;
            ...
            if(hnsw.gamma==1){
                ...
                for (size_t j = begin2; j < end2; j++) {
                    ...
                    // check filter pass
                    nfilter++;
                    if (sel->is_member(v2, pos)) {
                        num_found = num_found + 1;
                        float dis2 = qdis(v2);
                        ndis += 1;
                        nfilter++;
                        if (dis2 < d_nearest || !sel->is_member(nearest, pos)) {
                            nearest = v2;
                            d_nearest = dis2;
                            ngood++;
                        }
        ...
        if (nearest == prev_nearest) {
            ...
            stats.nfilter_first = nfilter;
            stats.nhop = nstep;
            stats.nhop_first = nstep;
            stats.ngood = ngood;
            stats.ngood_first = ngood;
            return stats;
        }
    }
}
...
// the original version of ACORN without optimization on repeated filtering
int hybrid_search_from_candidates(
    ...
    for (int i = 0; i < candidates.size(); i++) {
        ...
        #ifdef COUNT_USEFUL_RATES
        nr_visited++;
        if(visited_nodes.find(v1)==visited_nodes.end()){
            visited_nodes.insert(v1);
    ...
    while (candidates.size() > 0) { // candidates is heap of size max(efs, k)
        ...
        for (size_t j = begin; j < end; j++) {
            ...
            if(visited_nodes.find(v1)==visited_nodes.end()){
                visited_nodes.insert(v1);
                count_useful_queue.push(qdis(v1));
            }
            #endif
            ...
            if (((j - begin >= hnsw.M_beta) && keep_expanding) || hnsw.gamma == 1) {
                ...
                for (size_t j2 = begin2; j2 < end2; j2+=1) {
                    ...
                    #ifdef COUNT_USEFUL_RATES
                    nr_visited++;
                    if(visited_nodes.find(v2)==visited_nodes.end()){
                        visited_nodes.insert(v2);
                        count_useful_queue.push(qdis(v2));
                    }
                    #endif

                    if(params->output_search)
                        printf("[E] %f %d\n", qdis(v2), sel->is_member(v2, pos));
    ...
    if (level == 0) {
        ...
        float useful_rate = nr_visited ? 1.0 * nr_useful / nr_visited : 0;
...
int hybrid_search_from_candidates_filter_opt(
        const ACORN& hnsw,
        DistanceComputer& qdis,
        IDSelector* sel,
        int k,
    ...
    for (int i = 0; i < candidates.size(); i++) {
        ...
        float d = candidates.dis[i];
        FAISS_ASSERT(v1 >= 0);
        if (!sel || get_and_set_member(sel, vt, v1, nfilter, pos)) {
            if (nres < k) {
                faiss::maxheap_push(++nres, D, I, d, v1);
                ngood++;
            } else if (d < D[0]) {
    ...
    while (candidates.size() > 0) { // candidates is heap of size max(efs, k)
        ...
        for (size_t j = begin; j < end; j++) {
            ...
            bool pass_filter = false;
            bool prev_visted = vt.get_either(v1);
            if (get_and_set_member(sel, vt, v1, nfilter, pos)) {
               num_found = num_found + 1; // increment num found
               pass_filter = true;
            }

            #ifdef OUTPUT_SEARCH_PATH
            printf("%d\t%f\t%d\n", v1, qdis(v1), pass_filter?2:3);
            #endif
            #ifdef COUNT_USEFUL_RATES
            nr_visited++;
            if(visited_nodes.find(v1)==visited_nodes.end()){
            ...
            if (((j - begin >= hnsw.M_beta) && keep_expanding) || hnsw.gamma == 1) {
                ...
                for (size_t j2 = begin2; j2 < end2; j2+=1) {
                    ...
                    #ifdef OUTPUT_SEARCH_PATH
                    printf("%d\t%f\t%d\n", v1, qdis(v2), sel->is_member(v2, pos)?2:3);
                    #endif
                    #ifdef COUNT_USEFUL_RATES
                    nr_visited++;
                    if(visited_nodes.find(v2)==visited_nodes.end()){
                    ...
                    if (get_and_set_member(sel, vt, v2, nfilter, pos)) {
                        num_found = num_found + 1; // increment num found
                    } else {
                        continue;
                    }
...
// hybrid search limited mode, which does not expanding the neighbors
int hybrid_search_from_candidates_limited_mode(
        const ACORN& hnsw,
        DistanceComputer& qdis,
        IDSelector* sel,
        int k,
...
ACORNStats ACORN::search(
    ...
    if (upper_beam == 1) { // common branch
        ...
        for (int level = max_level; level >= 1; level--) {
            ACORNStats local_stats = greedy_update_nearest(*this, qdis, level, nearest, d_nearest);
...
ACORNStats ACORN::hybrid_search(
    ...
    // the search must have ID-selector
    IDSelector* sel = params->sel;
    FAISS_THROW_IF_NOT(sel);
    bool limited_mode = params->limited_mode;
    bool filter_opt = params ? params->filter_opt : false;
    ...
    if (upper_beam == 1) { // common branch
        ...
        for (int level = max_level; level >= 1; level--) {
            ...
            ACORNStats local_stats = hybrid_greedy_update_nearest(*this, qdis, sel, level, nearest, d_nearest, pos, params->output_search);
        ...
        if (search_bounded_queue) { // this is the most common branch
            ...
            if(limited_mode)
                hybrid_search_from_candidates_limited_mode(
                    *this, qdis, sel, k, I, D, candidates, vt, stats, 0, pos, 0, params);
            else{
                if(filter_opt)
                    hybrid_search_from_candidates_filter_opt(
                        *this, qdis, sel, k, I, D, candidates, vt, stats, 0, pos, 0, params);
                else
        ...
        for (int level = max_level; level >= 0; level--) {
            ...
            if (level == 0) {
                nres = hybrid_search_from_candidates(
                        *this, qdis, sel, k, I, D, candidates, vt, stats, pos, 0);
