#ifndef __FILTERED_HNSW_HPP__
#define __FILTERED_HNSW_HPP__
...
} filtered_hnsw_searchParameter;

class Filtered_HNSW_bencher : public Bencher {
private:
    // Search index.
...
public:
    Filtered_HNSW_bencher(std::string base_dir, std::string metadata_name) {
        if(logger)
            logger->logging("--- Testing Filtered_HNSW_bencher of " + base_dir + ": " + metadata_name + " ---\n", true);
        else
            printf("--- Testing Filtered_HNSW_bencher of %s:%s ---\n", base_dir.c_str(), metadata_name.c_str());
    ...
    Filtered_HNSW_bencher(std::string base_dir) {
        if(logger)
            logger->logging("--- Testing Filtered_HNSW_bencher of " + base_dir + " ---\n", true);
        else
            printf("--- Testing Filtered_HNSW_bencher of %s ---\n", base_dir.c_str());
        // Hybrid search: Load vector data and query data, as well as metadata, in bv form
    ...
    void build(void* buildParameter) {
        ...
        filtered_hnsw_buildParameter* param = (filtered_hnsw_buildParameter*) buildParameter;
        size_t M = param->M;
        ...
        index = new faiss::IndexHNSWFlat(dimension, M, faiss::METRIC_INNER_PRODUCT);

        if(param->efConstruction)
            index->hnsw.efConstruction = param->efConstruction;
    ...
    float query(void* searchParameter) {
        ...
        filtered_hnsw_searchParameter* param = (filtered_hnsw_searchParameter*) searchParameter;
        ...
        for(int i=0; i<nr_query; i++){
            ...
            search_param.output_search = param->output_search;
            ...
            stat_dist_comp.add(faiss::hnsw_stats.ndis);
            stat_dist_comp_first_stage.add(faiss::hnsw_stats.ndis_first);
            stat_nr_filter.add(faiss::hnsw_stats.nfilter);
            stat_nr_filter_first_stage.add(faiss::hnsw_stats.nfilter_first);
            stat_nr_step.add(faiss::hnsw_stats.nhop);
            stat_nr_step_first_stage.add(faiss::hnsw_stats.nhop_first);
            ...
            // printf("[summary] Query=%d recall=%f dist=%d filter=%d latency=%d\n", i, res, faiss::hnsw_stats.ndis, faiss::hnsw_stats.nfilter, end_time - start_time);
    ...
    float batch_query(void* searchParameter) {
        ...
        sprintf(output_content, "[Result] Nr_query=%u, total_time=%ld us, Tput=%.2f ops, avg_filter_first=%lu, avg_filter=%lu, avg_dist_first=%lu, avg_dist=%lu, avg_step_first=%lu, avg_step=%lu, avg_recall=%.2f %%\n", nr_query, (end_time-start_time), nr_query * 1e6 / (end_time - start_time), faiss::hnsw_stats.nfilter_first / nr_query, faiss::hnsw_stats.nfilter / nr_query, faiss::hnsw_stats.ndis_first / nr_query, faiss::hnsw_stats.ndis / nr_query, faiss::hnsw_stats.nhop_first / nr_query, faiss::hnsw_stats.nhop / nr_query, avg_recall * 100);
    ...
    float auto_batch_query(void* searchParameter, float target_recall, int min_efSearch=0, int max_efSearch=5000) {
        ...
        while(min_efSearch + 1 < max_efSearch){
            ...
            if(avg_recall <= target_recall){
                ...
                ndist_min = faiss::hnsw_stats.ndis;
                nfilter_min = faiss::hnsw_stats.nfilter;
            }
            else{
                max_efSearch = efSearch;
                recall_max_pos = avg_recall;
                time_max_pos = end_time - start_time;
                ndist_max = faiss::hnsw_stats.ndis;
                nfilter_max = faiss::hnsw_stats.nfilter;
            }
    ...
    void load_persist(char* persist_path){
        ...
        index = dynamic_cast<faiss::IndexHNSWFlat*>(faiss::read_index(persist_path));
