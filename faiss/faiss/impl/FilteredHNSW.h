...
struct FilteredHNSWStats;

struct SearchParametersFilteredHNSW : SearchParameters {
    int efSearch = 16;
    bool check_relative_distance = true;
    std::vector<char> filteredMap;

    ~SearchParametersFilteredHNSW() {}
};
...
struct FilteredHNSW {
    ...
    explicit FilteredHNSW(int M = 32, int gamma = 1);
    ...
    FilteredHNSWStats search(
            DistanceComputer& qdis,
            int k,
            idx_t* I,
            float* D,
            VisitedTable& vt,
            const SearchParametersFilteredHNSW* params = nullptr) const;
    ...
    **************************************************************/
    ...
    FilteredHNSWStats hybrid_search(
            DistanceComputer& qdis,
            int k,
            idx_t* I,
            float* D,
...
struct FilteredHNSWStats {
    ...
    void combine(const FilteredHNSWStats& other) {
        n1 += other.n1;
        n2 += other.n2;
        n3 += other.n3;
        ndis += other.ndis;
...
FAISS_API extern FilteredHNSWStats filteredhnsw_stats;
