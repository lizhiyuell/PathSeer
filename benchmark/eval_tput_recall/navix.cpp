...
int main() {
    ...
    std::vector<int> M_params_array = {32, 32, 64, 32, 64, 64};
    ...
    for(int ds_idx = 0; ds_idx < dataset_array.size(); ds_idx++){
        ...
        for(int test=0; test<3; test++){
            ...
            float recall_prev = NaviX_bencher.batch_query(&search_param);
            ...
            while(1){
                ...
                float recall = NaviX_bencher.batch_query(&search_param);
                // printf("%f\n", recall);
                ...
                if(recall > target_higher && (end_time - start_time)!=0 && (1.0 * NaviX_bencher.get_nr_query() / (end_time - start_time)) < min_tput){
                    break;
                }

                if(recall - recall_prev < 0.01 || recall >= 99)
                    break;
            ...
            for(float target_recall : target_recalls) {
                ...
                NaviX_bencher.auto_batch_query(&search_param, target_recall, lower_efSearch, higher_efSearch);
            }
