...
void IndexACORN::hybrid_search(
    ...
    size_t n1 = 0, n2 = 0, n3 = 0, ndis = 0, ndis_first = 0, nhop = 0, nhop_first = 0, nfilter = 0, nfilter_first = 0, ngood = 0, ngood_first = 0, nreorder = 0;
    float useful_rate = 0;
...
#pragma omp for reduction(+ : n1, n2, n3, ndis, ndis_first, nhop, nhop_first, nfilter, nfilter_first, ngood, ngood_first, nreorder, candidates_loop, useful_rate)
            for (idx_t i = i0; i < i1; i++) {
                ...
                // ACORNStats stats = acorn.hybrid_search(*dis, k, idxi, simi, vt, params); //TODO edit to hybrid search
                ACORNStats stats;
                if(params->sel)
                    stats = acorn.hybrid_search(*dis, k, idxi, simi, vt, i, params);
                else
                    stats = acorn.search(*dis, k, idxi, simi, vt, params);
    ...
    acorn_stats.combine({n1, n2, n3, ndis, ndis_first, nfilter, nfilter_first, nhop, nhop_first, ngood, ngood_first, nreorder, useful_rate, candidates_loop, neighbors_loop, tuple_unwrap, skips, visits}); //added for profiling
}
