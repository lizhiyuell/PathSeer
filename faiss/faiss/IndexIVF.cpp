...
#pragma omp parallel for if (nt > 1)
            ...
            if (i1 > i0) {
                try {
                    sub_search_func(
                            ...
                            distances + i0 * k,
                            labels + i0 * k,
                            &stats[slice]);//stats of current thread
        ...
        // collect stats
        for (idx_t slice = 0; slice < nt; slice++) {
            indexIVF_stats.add(stats[slice]);//After all threads have ended,add to indexIVF_stats
        }
    } else {
...
void IndexIVF::search_preassigned(
    ...
    size_t nlistv = 0, ndis = 0, nheap = 0, nfilter=0, nrealcomp=0;
...
#pragma omp parallel if (do_parallel) reduction(+ : nlistv, ndis, nheap, nfilter, nrealcomp)
    {
        ...
        auto scan_one_list = [&](idx_t key,
            ...
            try {
                ...
                if (selr) {
                    //IDSelectorRange:restrict search to a section of the inverted list
                    size_t jmin, jmax;
                ...
                IndexIVFStats local_stats;
                nheap += scanner->sel_scan_codes(
                        list_size, codes, ids, simi, idxi, k, &local_stats, pos);
                nfilter += local_stats.nfilter;
                nrealcomp += local_stats.nrealcomp;
...
#pragma omp single
    ...
    if (ivf_stats) {
        ...
        ivf_stats->nfilter += nfilter;
        ivf_stats->nrealcomp += nrealcomp;
    }
}
...
size_t InvertedListScanner::sel_scan_codes(
        size_t list_size,
        const uint8_t* codes,
        const idx_t* ids,
        float* simi,
